...
function cleanDistDirectory() {
  console.log(
    `${COLORS.green}- Step 1:${COLORS.stop} clear the ${outDirName} directory`
  );
...
function copyStaticFiles() {
  ...
  filesToCopyArr.forEach(
    ({ filename, sourceDirPath, destinationDirPath, isAllowedToFail }) => {
      try {
        ...
        const destinationFileFullPath = path.resolve(
          ROOT_PROJECT,
          outDirName,
          ...destinationDirPath,
          filename
        ...
        throw new Error(
          "File MUST exists in order to PASS build process! cp operation failed..."
        );
      }
    }
  );
...
function manipulatePackageJsonFile() {
  console.log(
    `${COLORS.green}- Step 4:${COLORS.stop} copy & manipulate the package.json file`
  );
  ...
  console.log(
    `  • ${COLORS.blue}changed${COLORS.stop} publishConfig access to public`
  );
  console.log(
    `  • ${COLORS.blue}package.json${COLORS.stop} file written successfully!`
  );
}
